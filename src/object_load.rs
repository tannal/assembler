...
impl SimpleJit {
    ...
    pub fn compile_read_balance(&mut self) -> (FuncId, JitFunction) {
        ...
        // 参数说明：
        // - i64_type: 我们要读出的数据类型
        // - MemFlags: 内存访问标记（如 trusted 表示指针合法）
        // - user_ptr: 基地址寄存器
        // - 8: 偏移量 (Offset)，跳过 8 字节的 id
        let balance = builder.ins().load(i64_type, MemFlags::new(), user_ptr, 8);

        // 3. 返回结果
        builder.ins().return_(&[balance]);
        
        builder.finalize();

        // ... 编译与返回逻辑 (与之前相同) ...
        let id = self.module.declare_function("object_load", Linkage::Export, &self.ctx.func.signature).unwrap();
